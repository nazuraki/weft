...
- **Anchor extractors** (`src/anchors/`) — Markdown heading slugs (via `github-slugger`), OpenAPI operation IDs + schema names. Anchors are `Anchor` objects (`slug`, `text`, and `line`/`level` for headings), not bare strings
